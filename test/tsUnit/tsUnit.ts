 ...
 * Update class to compliante es5 target replace class Set by Array in FunctionPropertyHelper class
 */
class Test {
...
class FunctionPropertyHelper {
    static addIfNotExist(results: Array<string>, value: string): number {
        if (results.indexOf(value) < 0) return results.push(value);
        return 0;
    }
    static walkImpl(obj: any, results: Array<string>): void {
        if (obj == null) {
            return;
        }
        const ownPropertiesOfObj = Object.getOwnPropertyNames(obj);
        ownPropertiesOfObj.forEach(mem => this.addIfNotExist(results,mem));
        const prototype = Object.getPrototypeOf(obj);
        if (prototype == null) {
            return null;
        }
        const propNames = Object.getOwnPropertyNames(prototype);
        propNames.forEach(mem => this.addIfNotExist(results, mem));
        this.walkImpl(obj.prototype, results);
        this.walkImpl(prototype, results);
    }
 
    static walk(obj: any): Array<string> {
